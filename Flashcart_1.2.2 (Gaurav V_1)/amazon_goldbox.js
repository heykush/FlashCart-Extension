...
s.innerHTML = `<source src="https://freesound.org/data/previews/109/109662_945474-lq.mp3" type="audio/mp3">`
...
function claim(dealid, asin, token) 
{
  ...
  $.ajax({
    ...
    success: function (data, textStatus, jQxhr) {
      ...
      if(data.isClaimed == "1"){
        inCart[dealid] = true
        //$('.toggle-switch').prop('checked', false); //Uncheck Switch
...
data = {"requestMetadata":{"marketplaceID":mid,"clientID":"goldbox_mobile_pc","customerID":cid,"sessionID":session_id},"dealTargets":dids,"responseSize":"STATUS_ONLY","itemResponseSize":"DEFAULT_WITH_PREEMPTIVE_LEAKING"}
  ...
  try{
    DealsArray.forEach(([key, value]) => { 
      ...
      if($(`span[data-gbdeal-addtocart*="${key}"],span[data-gbdeal-watchdeal*="${key}"],span[data-gbdeal-actionrecord*="${key}"]`).parents('.dealContainer').find('#flashcart').length < 1){
        ...
        if(x[key]){ //if Interval is running then set new switch to on
        ...
        $(`span[data-gbdeal-addtocart*="${key}"],span[data-gbdeal-watchdeal*="${key}"],span[data-gbdeal-actionrecord*="${key}"]`).parents('.dealContainer').find('.toggle-switch').change(function () {
          ...
          if ($(this).is(':checked')) {
            ...
            x[dealid] = setInterval(function () { //Execute infinite with delay
                if (inCart[dealid]) {
                  $(`span[data-gbdeal-addtocart*="${dealid}"],span[data-gbdeal-watchdeal*="${dealid}"],span[data-gbdeal-actionrecord*="${dealid}"]`).parents('.dealContainer').find('.toggle-switch').prop('checked', false);
                  clearInterval(x[dealid]);
                  console.log("Added to cart.")
      
                }
                else {
                    console.log("Trying to claim deal")
                    //$(`span[data-gbdeal-addtocart*="${dealid}"],span[data-gbdeal-watchdeal*="${dealid}"],span[data-gbdeal-actionrecord*="${dealid}"]`).parents('.dealContainer').find('.toggle-switch').prop('checked', true);
                    claim(dealid, asin, csrfToken)
                    //c += 1;
